...
type FilterStrategy = "all" | "SUI_MAXIMIZER" | "BTC_HODLER" | "STABLE_OPTIMIZER"
type SortOption = "roi" | "marketPrice" | "winRate" | "efficiency"

export default function ComparePage() {
  const [selectedAgents, setSelectedAgents] = useState<string[]>(["#1042", "#0987", "#1337"])
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="py-8">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          {/* Header */}
          <div className="mb-8">
            <h1 className="text-3xl md:text-4xl font-bold text-[#011829] mb-2">Compare Agents</h1>
            <p className="text-lg text-[#030F1C]">
              Evaluate different conviction-based AI agents before deciding to buy or copy.
            </p>
          </div>
          ...
          <div className="mb-8 flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
            <div className="flex flex-col sm:flex-row gap-4">
              <Select value={filterStrategy} onValueChange={(value: FilterStrategy) => setFilterStrategy(value)}>
              ...
              <Select value={sortBy} onValueChange={(value: SortOption) => setSortBy(value)}>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="efficiency">Strategy Efficiency</SelectItem>
                </SelectContent>
              </Select>
            </div>

            <Badge className="bg-[#4DA2FF]/10 text-[#4DA2FF]">Comparing {filteredAgents.length} agents</Badge>
          </div>
                  ...
                  <CardContent className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-[#030F1C] font-bold">{agent.marketPrice} USDC</p>
                      </div>
                      <div>
                    ...
                    <div className="space-y-3">
                      <div>
                        <div className="flex justify-between text-sm mb-1">
                          ...
                          <span className="font-medium text-[#011829]">{agent.winRate}%</span>
                        </div>
                      ...
                      <div>
                        <div className="flex justify-between text-sm mb-1">
                          <span className="text-[#030F1C]">Strategy Efficiency</span>
                          <span className="font-medium text-[#011829]">{agent.strategyEfficiency}%</span>
                        </div>
                        <Progress value={agent.strategyEfficiency} className="h-2" />
                      </div>
                    </div>

                    {/* Performance Sparkline */}
                    <div>
                      <p className="text-sm font-medium text-[#011829] mb-2">7-Day Trend</p>
                    ...
                    <div className="grid grid-cols-2 gap-4 text-sm">
                      <div>
                        ...
                        <p className="font-medium text-[#011829]">{agent.totalTrades}</p>
                      </div>
                      <div>
                        <p className="text-[#030F1C]">Avg Trade Size</p>
                        <p className="font-medium text-[#011829]">{agent.avgTradeSize} USDC</p>
                      </div>
                      <div>
                        <p className="text-[#030F1C]">Days Active</p>
                        <p className="font-medium text-[#011829]">{agent.daysActive}</p>
                      </div>
                      <div>
                        <p className="text-[#030F1C]">Owner</p>
                        <p className="font-mono text-xs text-[#4DA2FF]">{agent.owner}</p>
                      </div>
                    </div>
                    ...
                    <div className="space-y-2 pt-2">
                      ...
                      <div className="flex gap-2">
                        ...
                        <Link href={`/agent/${agent.id.slice(1)}/history`}>
                          <Button variant="outline" className="border-gray-300 text-[#030F1C] hover:bg-gray-50 text-sm">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Visual comparison of key performance indicators</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-6">
                {/* P&L Comparison */}
                <div>
                  <h4 className="font-medium text-[#011829] mb-3">Return on Investment (ROI)</h4>
                  <div className="space-y-3">
                    {filteredAgents.map((agent) => {
                      const { percentage } = calculatePnL(agent)
                      const maxRoi = Math.max(
                        ...filteredAgents.map((a) => ((a.currentValue - a.mintPrice) / a.mintPrice) * 100),
                      )
                      const barWidth = (Math.abs(percentage) / Math.abs(maxRoi)) * 100

                      return (
                        <div key={agent.id} className="flex items-center space-x-4">
                          <div className="w-20 text-sm font-medium text-[#011829]">{agent.id}</div>
                          <div className="flex-1 bg-gray-100 rounded-full h-4 relative">
                            <div
                              className={`h-4 rounded-full ${percentage >= 0 ? "bg-green-500" : "bg-red-500"}`}
                      ...
                      return (
                        <div key={agent.id} className="flex items-center space-x-4">
                          ...
                          <div className="flex-1 bg-gray-100 rounded-full h-4">
                            <div className="bg-blue-500 h-4 rounded-full" style={{ width: `${barWidth}%` }} />
                          </div>
                          <div className="w-16 text-sm font-medium text-right text-[#011829]">{agent.winRate}%</div>
                        </div>
                      )
                    })}
                  </div>
                ...
                <div>
                  <h4 className="font-medium text-[#011829] mb-3">Strategy Efficiency</h4>
                  <div className="space-y-3">
                    {filteredAgents.map((agent) => {
                      const maxEfficiency = getMaxValue("strategyEfficiency")
                      const barWidth = (agent.strategyEfficiency / maxEfficiency) * 100

                      return (
                        <div key={agent.id} className="flex items-center space-x-4">
                          <div className="w-20 text-sm font-medium text-[#011829]">{agent.id}</div>
                          <div className="flex-1 bg-gray-100 rounded-full h-4">
                            <div className="bg-purple-500 h-4 rounded-full" style={{ width: `${barWidth}%` }} />
                          </div>
